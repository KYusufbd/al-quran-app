...
const AlQuranPage = async () => {
  ...
  return (
    <div className="flex flex-col w-full bg-base-300">
      <div className="text-accent font-bold flex flex-col items-center p-6 bg-primary gap-2">
        ...
        {surahNamesAr.map((name, index) => {
          return (
            <Link key={index} href={`/al-quran/${index + 1}`}>
              <div className="w-full flex flex-col text-center justify-start items-center p-3 rounded-lg shadow-xl h-52 gap-5 bg-primary-content/80 text-primary font-normal">
                <h3 className="text-base mb-3">{index + 1}</h3>
                <h3 className="text-2xl font-medium font-amiri">{name}</h3>
