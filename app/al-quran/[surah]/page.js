...
export default async function Surah({ params }) {
  ...
  if (surah < 115 && surah > 0) {
    ...
    return (
      <div className="font-sans flex flex-col items-center min-h-screen px-1 pb-8 gap-16 bg-base-100">
            ...
            {verses.map((verse, index) => {
              ...
              return (
                <div id={ayah_no} className="flex flex-col gap-3 shadow-2xl rounded-xl p-2 pt-4 bg-primary-content/75" key={index}>
                  ...
                  <p className="text-3xl text-primary font-hafs leading-14 p-1 w-full rounded-xl bg-primary/10">{verse.indo_pak}</p>
                  {auth.length ? (
