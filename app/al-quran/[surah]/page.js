  ...
  if (surah < 115 && surah > 0) {
    ...
    return (
      <div
        id="surah"
        script_no={script_no}
        auth={JSON.stringify(authNumbers)}
        className="font-sans flex flex-col items-center min-h-screen px-1 pb-8 gap-16 bg-base-100"
      >
        <main className="flex flex-col gap-4 w-160 max-w-full items-center text-center">
          <div className="text-accent font-bold flex flex-col items-center p-6 bg-primary rounded-sm gap-4 w-full">
            <h1 className="text-4xl font-amiri font-medium">
              سُوْرَةُ {name_ar}
            </h1>
          ...
          <div className="flex flex-col gap-1 w-full">
            {verses.map((verse, index) => {
              ...
              return (
                <div
                  id={ayah_no}
                  className="flex flex-col gap-3 shadow-2xl rounded-xl p-2 pt-4 bg-primary-content/75"
                  key={index}
                >
